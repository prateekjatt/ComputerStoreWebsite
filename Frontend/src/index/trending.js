...
export default function Trending(){
    ...
    return (
        <div className="trending w-full p-12 text-center bg-black">
            <div className="trending-text"><h1 className=" text-4xl text-white tracking-widest">Trending</h1></div>
            ...
            <div className="cardcontainer flex justify-evenly items-center flex-wrap">
                {Array.from(cards).map((obj,idx)=>{
                    return <Card key={idx} image={obj.image} itemname={obj.item} price={obj.price}/>;
                })}
            </div>
        </div>
    );
